...
```{r}
# Read in PPIT output.
df_ppit <- read.table(file = "/Users/bennettkapili/Documents/STANFORD/DEKAS/nifH_Methods_Dev/tax_inference_pipeline/field_data/Analysis/df_inferences_mehta.txt", sep = "\t", row.names = 1, header = TRUE, fill = TRUE)

# Read in .csv containing the ASV names of those clustering with G. electrodiphilus, D. kysingii, and the Desulfuromonas metagenomes. Replace "." with "-", because for some reason these characters in the .csv file are changed upon import.
